...
function requestUpdateLane(fiber) {
  ...
  if (updateLane !== NoLane) {
    if ( (updateLane === InputContinuousLane || updateLane === InputContinuousHydrationLane)) {
      return DefaultLane;
    }
  ...
  if ( (eventLane === InputContinuousLane || eventLane === InputContinuousHydrationLane)) {
    return DefaultLane;
  }
...
function ensureRootIsScheduled(root, currentTime) {
  ...
  if ( (newCallbackPriority === DefaultLane || newCallbackPriority === DefaultHydrationLane)) {
...
function render(element, container, callback) {
  ...
  {
    // 这个应该是react 内置的增加某个随机的key 用来挂载组件树的
    // 
    var isModernRoot = isContainerMarkedAsRoot(container) && container._reactRootContainer === undefined;
